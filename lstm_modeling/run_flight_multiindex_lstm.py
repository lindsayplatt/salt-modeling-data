...
train_data_airlines_plot = train_data_airlines.reset_index(inplace=False)
valid_data_airlines_plot = valid_data_airlines.reset_index(inplace=False)
plt.figure(figsize=[12., 5.])
plt.plot(train_data_airlines_plot.level_0, train_data_airlines_plot.value, 'b--', label='Train', )
plt.plot(valid_data_airlines_plot.level_0, valid_data_airlines_plot.value, 'g--', label='Valid', )
plt.legend()
...
# Separate by airline
train_data_out = train_data_airlines_plot
train_data_out['preds'] = train_preds
train_preds_american = train_data_out.query('airline == "american"')
train_preds_delta = train_data_out.query('airline == "delta"')

valid_data_out = valid_data_airlines_plot[:-1]
valid_data_out['preds'] = valid_preds
valid_preds_american = valid_data_out.query('airline == "american"')
valid_preds_delta = valid_data_out.query('airline == "delta"')

data_out = data.reset_index(inplace=False)
data_out_american = data_out.query('airline == "american"')
data_out_delta = data_out.query('airline == "delta"')
...
ax1.plot(train_preds_american.level_0, train_preds_american.preds, 'r--', label='Training Predictions, American', )
ax2.plot(train_preds_delta.level_0, train_preds_delta.preds, 'r--', label='Training Predictions, Delta', )
ax1.plot(valid_preds_american.level_0, valid_preds_american.preds, 'g--', label='Validation Predictions, American', )
ax2.plot(valid_preds_delta.level_0, valid_preds_delta.preds, 'g--', label='Validation Predictions, Delta', )
ax1.plot(data_out_american.level_0, data_out_american.value, label='Observations, American')
ax2.plot(data_out_delta.level_0, data_out_delta.value, label='Observations, Delta')
