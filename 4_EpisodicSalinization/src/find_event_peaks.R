...
#' of values. Unsure if data needs to be gap-free, but thinking that it should.
#' Also unsure if we should be using normalized data here.
...
find_event_peaks <- function(ts_data, date_colname, param_colname, sb_pk_thresh = 0.000001, sf_pk_thresh = 0){
  ...
  # TODO: this assumes that the data is continuous.
  # We should use the gap-filled version, I think.
  
  ts_data <- ts_data %>% 
    # Convert to seconds
    mutate(Date := as.POSIXlt(!!as.name(date_colname), format = '%m/%d/%y'),
           Seconds := as.numeric(!!as.name(date_colname))) %>%
    ...
    filter(!is.na(PARAM)) %>% 
    # filter(PARAM > 0) %>% # TODO: THIS WILL NOT PLAY NICELY WITH THE NORMALIZED DATA
    # Make sure plotting variable is greater than zero
    # Calculate slope forward and backward
    mutate(slope_bkwd = (PARAM - lag(PARAM)/(Seconds - lag(Seconds))),
           slope_fwd = (lead(PARAM)-PARAM)/(lead(Seconds)-Seconds)) %>% 
    ...
    mutate(peak_flag = ifelse(slope_bkwd > sb_pk_thresh & slope_fwd < sf_pk_thresh,
                              yes = 1, no = 0))
  ...
  events <- which(ts_data$peak_flag == 1)
  ...
  if(length(events) == 0){
    ...
    ts_data$event.flag <- rep(NA, length.out = nrow(ts_data))
